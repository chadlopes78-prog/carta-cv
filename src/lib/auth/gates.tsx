...
export function SignInButtons() {
  return (
    <div className="flex w-full max-w-sm flex-col gap-2">
      <a
        href="/login"
        className="inline-flex h-11 w-full items-center justify-center rounded-md bg-forest text-sm font-medium text-cream"
      >
...
export function UserButton({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={cn("truncate text-sm font-medium", compact && "max-w-20 sm:max-w-none")}>{compact ? label.split(" ")[0] : label}</span>
